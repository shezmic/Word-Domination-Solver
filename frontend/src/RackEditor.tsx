...
export const RackEditor: React.FC = () => {
  const { rack, updateRack, setRackFromText, rackSize } = useSolverStore();
  const [rackText, setRackText] = React.useState('');
  
  // Count non-empty tiles
  const tileCount = rack.filter(t => t > 0).length;
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">{tileCount}/{rackSize} tiles</p>
        </div>
      </div>
